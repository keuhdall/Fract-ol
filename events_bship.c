...
/*   Updated: 2016/12/08 14:47:36 by lmarques         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	ft_init_bship(&f);
	ft_draw_bship(&(f), 800, 600);
	mlx_put_image_to_window(f.mlx.ptr, f.mlx.win, f.mlx.img, 0, 0);
	mlx_hook(f.mlx.win, 17, 1L << 17, &ft_exit_sig, &f);
	mlx_mouse_hook(f.mlx.win, &ft_mouse_events_bs, &f);
	mlx_key_hook(f.mlx.win, &ft_keyboard_events_bs, &f);
	mlx_loop(f.mlx.ptr);
}
